...
public class SimpleGame<I, S> extends AbstractGame<I, S> {
	...
	private final Map<String, SoundObject<S>> sounds = new HashMap<>();
	public SimpleGame(Runnable quit) {
		// init with player
		...
		for (String name : new String[]{"pop", "axe1", "axe2", "axe3", "axe4", "arrow", "arrow_silent", "coin", "coins", "walking"})
			sounds.put(name, new SoundObject<>(name + ".wav"));
	...
	public void pSound(String key) {
		if(muteSound) return;
		SoundObject<S> s = sounds.get(key);
		playSound(s);
	}
	public void resetLvl(int levelID) {
		if(levelID == -1) this.quit.run();
		items.clear();
		blocks.clear();
		pushables.clear();
	...
	public void doChecks() {
				...
				for (GameObject<I> b : tmp) {
					if (b.touches(item)) {
						if (item.getVelocity().dist() > 0) {
							if(((Arrow<I>) item).stuckIn == null) pSound("arrow_silent");
		...
		if(player.getPos().y < -100 && player.getDeathTimer() == 0) {
			...
			pSound("walking"); // ggf auskommentieren in Linux
		}
		moveViewport();
	}
	...
	public boolean skeletonKi(Skeleton<I> s) { // return false to remove
		...
		if(s.getMood() == 3 && s.getCurrentAnimationFrame() == 4 && s.getAnimationFrameSkip() == 0) {
			pSound("axe" + (int)(Math.random()*4+1));
		}
